                        ...
                        <tbody>
                         @foreach($roles as $role)
                            <tr>
                                ...
                                <td class="bg-light"><span class="text-gray-3">{{ $role->created_at->format('d M Y') }}</span></td>
                                 <td class="bg-light">
                                    <a href="javascript:void(0);" class="btn btn-outline-white d-inline-flex align-items-center" data-bs-toggle="modal" data-bs-target="#assignPermissionsModal{{ $role->id }}">
                                ...
                                <td class="action-item bg-light">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a href="javascript:void(0);" class="dropdown-item d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#editRole{{ $role->id }}"><i class="isax isax-edit me-2"></i>Edit</a>
                                        </li>
                                        <li>
                                            <a href="javascript:void(0);" class="dropdown-item d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#deleteRole{{ $role->id }}"><i class="isax isax-trash me-2"></i>Delete</a>
                                        </li>
                                    </ul>
                                </td>
                            </tr>
...
<div id="editRole{{ $role->id }}" class="modal fade">
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            ...
            <form action="{{ route('roles.updateRole', ['role' => $role->id]) }}" method="POST">
    @csrf
    @method('PUT')
...
<div class="modal fade" id="deleteRole{{ $role->id }}">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <form action="{{ route('roles.destroyRole', ['role' => $role->id]) }}" method="POST">
    @csrf
    @method('DELETE')
    ...
    <div class="modal-body">
        <div class="mb-0">
            <label class="form-label">Permissions</label>
              <select class="select2 form-control select2-multiple" name="permissions[]" multiple="multiple" data-placeholder="Choose permissions...">
