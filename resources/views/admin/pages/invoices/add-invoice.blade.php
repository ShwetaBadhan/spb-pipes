            ...
            <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <div id="discount-percent-section">
                        <input type="number" name="discount_percent" class="form-control" value="0" step="0.01" min="0" max="100" id="discount-percent-input">
                        <small class="text-muted">Enter discount percentage (0-100%)</small>
                    </div>
                    
                    <div id="discount-amount-section" style="display: none;">
                        <input type="number" name="discount_amount" class="form-control" value="0" step="0.01" min="0" id="discount-amount-input">
                        <small class="text-muted">Enter discount amount in rupees</small>
                    </div>
                </div>
                ...
                <div class="d-flex align-items-center justify-content-between border-bottom pb-3 mb-3">
                    <div class="form-check form-switch me-4">
                        ...
                        <input class="form-check-input" type="checkbox" role="switch" id="round_off" name="round_off" value="1">
...
document.getElementById('tax_type').addEventListener('change', function() {
    ...
    document.addEventListener('input', function(e) {
        const row = e.target.closest('tr');
        if (row && ['quantity-input', 'rate-input', 'discount-input'].some(cls => e.target.classList.contains(cls))) {
            calculateRow(row);
            calculateTotals();
        }
        // ✅ CRITICAL FIX #2: Include BOTH discount inputs
        if (['shipping-input', 'discount-percent-input', 'discount-amount-input'].includes(e.target.id)) {
            calculateTotals();
        }
    });
    ...
    function calculateTotals() {
        ...
        if (enableTax && taxType !== 'none') {
            switch (taxType) {
                case 'gst_5': totalTax = (subtotal * 5) / 100; addTaxRow('GST 5%', totalTax); break;
                case 'gst_12': totalTax = (subtotal * 12) / 100; addTaxRow('GST 12%', totalTax); break;
                case 'gst_18': totalTax = (subtotal * 18) / 100; addTaxRow('GST 18%', totalTax); break;
                case 'gst_28': totalTax = (subtotal * 28) / 100; addTaxRow('GST 28%', totalTax); break;
                case 'cgst_sgst':
                    ...
                    break;
                case 'igst': totalTax = (subtotal * 18) / 100; addTaxRow('IGST 18%', totalTax); break;
            }
        }
