...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // ✅ SEND EMAIL NOTIFICATION (After commit - non-blocking)
            try {
    ...
    // ✅ HELPER METHOD: Send invoice notification email
  /**
 * Send invoice notification email to admin
 */
protected function sendInvoiceNotification(Invoice $invoice)
{
    // Get admin email with safer fallback chain
    $adminEmail = config('app.admin_email') 
                ?? env('ADMIN_EMAIL') 
                ?? $this->getFallbackAdminEmail();
    
    // ✅ Validate email format BEFORE attempting to send
    if (!filter_var($adminEmail, FILTER_VALIDATE_EMAIL)) {
        Log::warning('Invalid admin email address - skipping notification', [
            'invoice_number' => $invoice->invoice_number,
    ...
    // Prevent sending to localhost/internal domains
    $domain = strstr($adminEmail, '@', false);
    if (in_array(strtolower($domain), ['@localhost', '@local', '@test', '@example.com', '@example.org'])) {
        Log::warning('Admin email uses invalid domain - skipping notification', [
            'invoice_number' => $invoice->invoice_number,
            'email' => $adminEmail
        ]);
        return;
    ...
    // Configure mailer from active EmailSetting
    if (!$this->configureMailerFromSettings()) {
        Log::warning('No active email configuration found for invoice notification', [
            'invoice_number' => $invoice->invoice_number
        ]);
        return;
    }
    ...
    // Send the email
    Mail::send(new InvoiceCreatedMail($invoice, $adminEmail));
...
/**
 * Get a safe fallback admin email
 */
protected function getFallbackAdminEmail(): string
{
    // Try to get from authenticated admin user
    if (Auth::check() && Auth::user()->email && filter_var(Auth::user()->email, FILTER_VALIDATE_EMAIL)) {
    ...
    // Last resort: use a hardcoded safe default (change this!)
    return 'admin@vibrantick.in';
}
